...
func save_game() -> void:
	...
	save_file.set_value(game.SAVE_SECTION, game.SAVE_SCORE_BIG_KEY, game.score_counter.to_decimal_string())
	save_file.set_value(game.SAVE_SECTION, game.SAVE_COINS_BIG_KEY, game.coins_counter.to_decimal_string())
	...
	save_file.set_value(game.SAVE_SECTION, game.SAVE_BEST_COIN_BALANCE_BIG_KEY, game.best_coin_balance_counter.to_decimal_string())
	...
	save_file.set_value(game.SAVE_SECTION, game.SAVE_MASTER_VOLUME_KEY, game.master_volume)
	save_file.set_value(game.SAVE_SECTION, game.SAVE_CLICK_SOUNDS_ENABLED_KEY, game.click_sounds_enabled)
	save_file.set_value(game.SAVE_SECTION, game.SAVE_UI_SOUNDS_ENABLED_KEY, game.ui_sounds_enabled)
	save_file.set_value(game.SAVE_SECTION, game.SAVE_MUTE_UNFOCUSED_KEY, game.mute_unfocused)
	save_file.set_value(game.SAVE_SECTION, game.SAVE_LOW_QUALITY_ENABLED_KEY, game.low_quality_enabled)
	save_file.set_value(game.SAVE_SECTION, game.SAVE_BATTERY_SAVER_ENABLED_KEY, game.battery_saver_enabled)
	save_file.set_value(game.SAVE_SECTION, game.SAVE_OPTIMIZED_TAP_EFFECTS_KEY, game.optimized_tap_effects)
	save_file.set_value(game.SAVE_SECTION, game.SAVE_REDUCE_MOTION_ENABLED_KEY, game.reduce_motion_enabled)
	save_file.set_value(game.SAVE_SECTION, game.SAVE_BACKGROUND_EFFECTS_ENABLED_KEY, game.background_effects_enabled)
	save_file.set_value(game.SAVE_SECTION, game.SAVE_LOW_POWER_UNFOCUSED_KEY, game.low_power_unfocused)
	...
	save_file.set_value(game.SAVE_SECTION, game.SAVE_HAPTIC_STRENGTH_KEY, game.haptic_strength)
	save_file.set_value(game.SAVE_SECTION, game.SAVE_EVENTS_ENABLED_KEY, game.events_enabled)
	save_file.set_value(game.SAVE_SECTION, game.SAVE_FLOATING_NUMBERS_ENABLED_KEY, game.floating_numbers_enabled)
	save_file.set_value(game.SAVE_SECTION, game.SAVE_COIN_TRAILS_ENABLED_KEY, game.coin_trails_enabled)
	save_file.set_value(game.SAVE_SECTION, game.SAVE_MENU_SWIPE_ENABLED_KEY, game.menu_swipe_enabled)
	save_file.set_value(game.SAVE_SECTION, game.SAVE_REVERSE_SLIDERS_ENABLED_KEY, game.reverse_sliders_enabled)
	save_file.set_value(game.SAVE_SECTION, game.SAVE_SLIDER_SOUND_STYLE_KEY, game.slider_sound_style)
	save_file.set_value(game.SAVE_SECTION, game.SAVE_ABBREVIATE_NUMBERS_KEY, game.abbreviate_numbers)
	save_file.set_value(game.SAVE_SECTION, game.SAVE_NUMBER_DETAIL_DIGITS_KEY, game.number_detail_digits)
	save_file.set_value(game.SAVE_SECTION, game.SAVE_EXACT_NUMBER_TOOLTIPS_KEY, game.exact_number_tooltips)
	save_file.set_value(game.SAVE_SECTION, game.SAVE_GROUP_FULL_NUMBERS_KEY, game.group_full_numbers)
...
func load_game() -> void:
	...
	var legacy_score := maxi(0, int(save_file.get_value(game.SAVE_SECTION, game.SAVE_SCORE_KEY, 0)))
	var legacy_coins := maxi(0, int(save_file.get_value(game.SAVE_SECTION, game.SAVE_COINS_KEY, legacy_score)))
	if not game.score_counter.set_from_decimal_string(String(save_file.get_value(game.SAVE_SECTION, game.SAVE_SCORE_BIG_KEY, str(legacy_score)))):
		game.score_counter.set_from_int(legacy_score)
	if not game.coins_counter.set_from_decimal_string(String(save_file.get_value(game.SAVE_SECTION, game.SAVE_COINS_BIG_KEY, str(legacy_coins)))):
		game.coins_counter.set_from_int(legacy_coins)
	game.score = game.score_counter.to_clamped_int(game.MAX_RESOURCE_VALUE)
	game.coins = game.coins_counter.to_clamped_int(game.MAX_RESOURCE_VALUE)
	...
	var legacy_best_coins := maxi(0, int(save_file.get_value(game.SAVE_SECTION, game.SAVE_BEST_COIN_BALANCE_KEY, game.coins)))
	if not game.best_coin_balance_counter.set_from_decimal_string(String(save_file.get_value(game.SAVE_SECTION, game.SAVE_BEST_COIN_BALANCE_BIG_KEY, str(legacy_best_coins)))):
		game.best_coin_balance_counter.set_from_int(legacy_best_coins)
	game.best_coin_balance = game.best_coin_balance_counter.to_clamped_int(game.MAX_RESOURCE_VALUE)
	...
	if not game.update_095_balance_migration_applied:
		game.score_counter.set_from_int(mini(game.score, game.UPDATE_095_RESOURCE_CAP))
		game.coins_counter.set_from_int(mini(game.coins, game.UPDATE_095_RESOURCE_CAP))
		game.best_coin_balance_counter.set_from_int(mini(game.best_coin_balance, game.UPDATE_095_RESOURCE_CAP))
		game.update_095_balance_migration_applied = true
...
func apply_offline_gain(last_seen_unix: int) -> void:
	...
	if game.last_offline_minutes <= 0:
		return
	var base_offline_gain := float(game.last_offline_minutes) * float(game._get_effective_passive_gain())
	game.last_offline_gain = game._safe_resource_round(base_offline_gain * game._get_global_gain_multiplier())
...
func get_offline_info_text() -> String:
	...
	var max_gain: int = game._safe_resource_round(float(game._get_effective_passive_gain()) * float(max_minutes) * game._get_global_gain_multiplier())
